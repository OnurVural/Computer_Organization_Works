...
DuplicateListRecursive:
	...
	# SAFE GUARD FOR EMPTY PASSING
	beq	$a0, $zero, pass
	jal DuplicateListRecursive_
	sw	$zero, 0($v0)
	addi	$v0, $v0, 8
	pass:
	lw	$ra, 0($sp)
...
#-------------------------------------------------------------------------------------------------------
	...
	move	$s1, $a0
bne	$a0, $zero, else
	#add	$s2, $zero, $zero # return null(0)
	#move	$s2, $zero
	#sw	$s2, 0($v0)
	#addi	$v0, $v0, 8
	addi 	$sp, $sp, 20
...
#-------------------------------------------------------------------------------------------------------	
	...
	move	$s2, $v0 # get address of current node for keeping in mind
	...
	#sw	$v0, 0($v0) # Link the node to the next by inserting its address
	
	lw	$a0, 0($a0) # pass to next node
	jal	DuplicateListRecursive_ #+++++++++++++++REC->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	
	subi	$v0, $v0, 8 # pass previous node
	
	sw	$s2, 0($v0) # get next address
